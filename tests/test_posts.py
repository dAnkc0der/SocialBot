...
@pytest.mark.parametrize("title, content, published", [ 
    ("first title -no", "first content -no", True),
    ("fastapi", "about to finish fastapi", True),
    ("rest framework", "started", False),
])
def test_create_post(authorized_client, test_user, test_posts, title, content, published):
    ...
    assert str(created_post.owner_id) == test_user['id']

def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title": "Bruh-title", "content" : "content"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "Bruh-title"
    assert created_post.content == "content"
    assert created_post.published == True
    assert str(created_post.owner_id) == test_user['id']

def test_unauthorized_user_create_posts(client, test_posts, test_user):
    res = client.post("/posts/", json={"title": "Bruh-title", "content" : "content"})
    assert res.status_code == 401
